...
In 1835, following the abolition of slavery throughout the British Empire, the accumulated sum was directed by Sir Thomas Fowell Buxton and his fellow reformers toward a new and enduring purpose — the education of the newly emancipated peoples of the West Indies. From that reimagined bequest sprang the Lady Mico Charity, and with it the teachers’ colleges and schools whose legacy The Mico Foundation carries forward today.`,
    ...
    defineField({
      ...
      description: 'Image shown in the left column beneath the paragraph.',
    }),
    defineField({
      name: 'rightImage',
      title: 'Right Image',
      type: 'image',
      options: { hotspot: true },
      description: 'Image shown in the right column beneath the paragraph.',
    }),
    defineField({
      name: 'rightNarrative',
      title: 'Right Column — Narrative (below the right image)',
      type: 'text',
      rows: 6,
      description: 'Optional text shown beneath the right image. Separate paragraphs with a blank line.',
    }),
    defineField({
      name: 'image',
      title: 'Legacy Centered Image (deprecated)',
      type: 'image',
      options: { hotspot: true },
      hidden: true,
      description: 'No longer used — replaced by Left Image / Right Image.',
    }),
  ],
  preview: {
    prepare() {

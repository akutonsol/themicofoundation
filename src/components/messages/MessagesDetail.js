...
const messages = [
  {
    ...
    image:
      "https://www.micofoundation.org/wp-content/uploads/2023/09/Dr.-Sylvester-Tulloch-CD.jpg",
    intro:
    ...
    body: [
      "The Mico Foundation continues to play a vital role in safeguarding the legacy, mission, and future development of The Mico University College. Our work is rooted in service, stewardship, and a deep commitment to advancing education across Jamaica and the wider Caribbean.",
      ...
      "We are grateful to our donors, alumni, trustees, partners, and friends who continue to support the Foundation’s work. Your generosity allows us to preserve history while building pathways for tomorrow’s leaders.",
    ],
  },
  {
    ...
    intro:
      "The work of The Mico Foundation is deeply connected to educational stewardship, legacy, and service.",
    body: [
  ...
  },
  {
    ...
    image:
      "https://www.micofoundation.org/wp-content/uploads/2023/09/Dr.-Sylvester-Tulloch-CD.jpg",
    intro:
      "Leadership, service, and education remain at the center of our work as a Foundation.",
    body: [
...
function BackgroundGrid() {
  return (
    <div className="pointer-events-none absolute inset-0 overflow-hidden">
      ...
      <div className="absolute left-[6%] top-[18%] text-[42px] text-[#FFD900]">
        ✦
      </div>

      <div className="absolute right-[12%] bottom-[18%] text-[42px] text-[#FFD900]">
        ✦
      </div>
    </div>
  );
...
export default function MessageDetail() {
  ...
  const slug = searchParams.get("slug") || messages[0].slug;

  const currentIndex = messages.findIndex((item) => item.slug === slug);
  const safeIndex = currentIndex === -1 ? 0 : currentIndex;
  const message = messages[safeIndex];

  const prevMessage = safeIndex > 0 ? messages[safeIndex - 1] : null;
  const nextMessage =
    safeIndex < messages.length - 1 ? messages[safeIndex + 1] : null;
    ...
    <main className="relative overflow-hidden bg-[#FAF9F6]">
      ...
      <section className="relative px-6 py-12 sm:px-10 lg:px-20 lg:py-20">
        <div className="mx-auto max-w-[1650px]">
          <Link
            href="/"
            className="inline-flex items-center gap-2 text-[15px] text-[#040617]"
            style={inter}
          >
        ...
        >
          ...
          <div className="mt-20 flex flex-col gap-6 border-t border-black/10 pt-8 sm:flex-row sm:items-center sm:justify-between">
            <div>
              {prevMessage ? (
                <Link
                  href={`/messagedetail?slug=${prevMessage.slug}`}
            ...
            <div>
              {nextMessage ? (
                <Link
                  href={`/messagedetail?slug=${nextMessage.slug}`}
                  className="inline-flex items-center gap-4"
                >
                  <span className="flex h-[54px] w-[54px] items-center justify-center rounded-[14px] bg-[#E5E6EB] text-[#040617] transition hover:bg-[#FFD900]">

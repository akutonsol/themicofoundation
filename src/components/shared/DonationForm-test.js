...
function CardTypeIcon({ type }) {
  ...
  if (type === "mastercard") return (
    <span style={{display:"inline-flex",alignItems:"center",gap:"1px"}}>
      <span style={{width:"20px",height:"20px",borderRadius:"50%",backgroundColor:"#EB001B",display:"inline-block"}}/>
      <span style={{width:"20px",height:"20px",borderRadius:"50%",backgroundColor:"#F79E1B",display:"inline-block",marginLeft:"-8px"}}/>
    </span>
  );
...
const COUNTRIES = [
  ...
  { code:"IN", name:"India", states:["Andhra Pradesh","Assam","Bihar","Delhi","Goa","Gujarat","Haryana","Himachal Pradesh","Jharkhand","Karnataka","Kerala","Madhya Pradesh","Maharashtra","Manipur","Meghalaya","Mizoram","Nagaland","Odisha","Punjab","Rajasthan","Sikkim","Tamil Nadu","Telangana","Tripura","Uttar Pradesh","Uttarakhand","West Bengal"] },
...
const formatCurrencyCode = (countryCode) => {
  const map = { US:"840", JM:"388", CA:"124", GB:"826" };
  return map[countryCode] || "840";
};
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"16px",width:"100%"}}>
      {steps.map((s,i)=>(
        <div key={s.num}>
          ...
          {i<steps.length-1&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",height:"24px",marginTop:"4px"}}><svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M6 9L12 15L18 9" stroke="#6F7181" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" opacity="0.5"/></svg></div>}
        </div>
      ))}
    </div>
  );
    ...
    <div style={{display:"flex",alignItems:"center",gap:"32px",maxWidth:"1320px",margin:"0 auto 36px"}}>
      {steps.map((s,i)=>(
        <div key={s.num} style={{display:"flex",flex:1,alignItems:"center",gap:"16px",opacity:s.num===display?1:0.5}}>
          ...
          {i<steps.length-1&&<svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M9 18L15 12L9 6" stroke="#6F7181" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" opacity="0.5"/></svg>}
        </div>
      ))}
    </div>
  );
    ...
    <div style={{display:"flex",flexDirection:"column",gap:"20px",width:"100%",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{width:"100%",aspectRatio:"16/9",borderRadius:"12px",overflow:"hidden",position:"relative",zIndex:1}}>
        ...
        <div style={{position:"absolute",top:"12px",left:"50%",transform:"translateX(-50%)",display:"flex",alignItems:"center",gap:"8px",whiteSpace:"nowrap"}}>
          <img src={imgLocation} alt="" style={{width:"24px",height:"24px"}}/><span style={{...inter,fontSize:"16px",color:"white"}}>{p.location}</span>
        </div>
      </div>
      ...
      {projects.length>1&&<div style={{display:"flex",alignItems:"center",justifyContent:"center",gap:"32px",position:"relative",zIndex:1}}>
        <button onClick={onPrev} style={{width:"44px",height:"44px",border:"2px solid #040617",borderRadius:"12px",display:"flex",alignItems:"center",justifyContent:"center",background:"none",cursor:"pointer",opacity:0.6}}><svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M15 18L9 12L15 6" stroke="#040617" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
        ...
        <button onClick={onNext} style={{width:"44px",height:"44px",backgroundColor:"#040617",borderRadius:"12px",display:"flex",alignItems:"center",justifyContent:"center",border:"none",cursor:"pointer"}}><svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M9 18L15 12L9 6" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
      </div>}
    </div>
  );
  return (
    <div style={{backgroundColor:"#FFFDF9",border:"1px solid #E5E6EB",borderRadius:"20px",padding:"16px",display:"flex",flexDirection:"column",gap:"32px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"8px"}}><img src={imgLocation} alt="" style={{width:"24px",height:"24px"}}/><span style={{...inter,fontSize:"16px",color:"#6F7181"}}>{p.location}</span></div>
      </div>
      ...
      {projects.length>1&&<div style={{display:"flex",alignItems:"center",justifyContent:"space-between",position:"relative",zIndex:1}}>
        <div style={{display:"flex",alignItems:"center",gap:"16px",opacity:0.6}}>
          ...
          <button onClick={onPrev} style={{width:"44px",height:"44px",border:"2px solid #040617",borderRadius:"12px",display:"flex",alignItems:"center",justifyContent:"center",background:"none",cursor:"pointer"}}><svg width="24" height="24" viewBox="0 0 24 24" fill="none"><path d="M15 18L9 12L15 6" stroke="#040617" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg></button>
        </div>
...
// ── Step 1 — Amount ───────────────────────────────────────────────────────────
function AmountStep({ tab, setTab, selected, setSelected, custom, setCustom, error, onNext, mobile=false }) {
  ...
  if (mobile) return (
    <div style={{width:"100%",backgroundColor:"#FFFDF9",borderRadius:"16px",overflow:"hidden",border:"1px solid #E5E6EB",display:"flex",flexDirection:"column"}}>
      <div style={{display:"flex",borderBottom:"1px solid #E5E6EB"}}>
        <button style={{flex:1,padding:"10px",...inter,fontSize:"24px",lineHeight:"38px",color:"#040617",background:"none",border:"none",borderBottom:tab==="once"?"2px solid #FFD900":"2px solid transparent",cursor:"pointer",opacity:tab==="once"?1:0.8}} onClick={()=>{setTab("once");setSelected("$50")}}>Once</button>
        <button style={{flex:1,padding:"10px",...inter,fontSize:"24px",lineHeight:"38px",color:"#040617",background:"none",border:"none",borderBottom:tab==="monthly"?"2px solid #FFD900":"2px solid transparent",cursor:"pointer",opacity:tab==="monthly"?1:0.8}} onClick={()=>{setTab("monthly");setSelected("$50/Month")}}>Monthly</button>
      </div>
      <div style={{padding:"24px",display:"flex",flexDirection:"column",gap:"16px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
          {amounts.map(a=><button key={a} onClick={()=>setSelected(a)} style={{cursor:"pointer",border:`1px solid ${selected===a?"#FFD900":"#E5E6EB"}`,background:selected===a?"#FFD900":"#FFFDF9",borderRadius:"12px",padding:"10px 32px",...inter,fontSize:"20px",color:"#040617",textAlign:"center",transition:"all 0.2s"}}>{a}</button>)}
        </div>
        <button onClick={()=>setSelected("custom")} style={{cursor:"pointer",border:`1px solid ${selected==="custom"?"#FFD900":"#E5E6EB"}`,background:selected==="custom"?"#FFD900":"#FFFDF9",borderRadius:"12px",padding:"10px 32px",...inter,fontSize:"20px",color:"#040617",textAlign:"center"}}>Custom</button>
        {selected==="custom"&&<div><input type="text" placeholder="Enter amount" value={custom} onChange={e=>setCustom(e.target.value.replace(/[^0-9.]/g,""))} style={{...inter,width:"100%",border:`1px solid ${error?"#EF4444":"#FFD900"}`,borderRadius:"12px",padding:"12px 16px",fontSize:"18px",color:"#040617",outline:"none",boxSizing:"border-box"}}/>{error&&<p style={{...inter,fontSize:"14px",color:"#EF4444",margin:"4px 0 0"}}>{error}</p>}</div>}
        ...
        <button onClick={onNext} style={{...inter,display:"flex",alignItems:"center",justifyContent:"center",gap:"12px",backgroundColor:"#FFD900",color:"#040617",fontSize:"16px",fontWeight:600,padding:"16px 24px",borderRadius:"18px",border:"none",cursor:"pointer",width:"100%"}}>
          Continue to Personal Info <img src={imgArrowBtn} alt="" style={{width:"24px",height:"24px"}}/>
        </button>
      </div>
      <div style={{backgroundColor:"#E8E9EB",padding:"24px"}}>
        <p style={{...inter,fontSize:"20px",color:"#636473",lineHeight:"30px",margin:0,textAlign:"center"}}>Your <strong style={{color:"#040617"}}>{selected==="custom"?(custom||"$50"):selected}</strong> donation can help equip up to <strong style={{color:"#040617"}}>20</strong> students with school supplies, access to technology, and a safe learning environment.</p>
      </div>
    </div>
  );
  return (
    <div style={{backgroundColor:"#FFFDF9",border:"1px solid #E5E6EB",borderRadius:"20px",overflow:"hidden",display:"flex",flexDirection:"column",position:"relative"}}>
      ...
      <div style={{padding:"24px",display:"flex",flexDirection:"column",flex:1,gap:"24px",position:"relative",zIndex:1}}>
        <div style={{display:"flex",flexDirection:"column",gap:"16px"}}>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"20px"}}>
            {amounts.map(a=><button key={a} className={`amt-btn${selected===a?" active":""}`} onClick={()=>setSelected(a)}>{a}</button>)}
          </div>
          <div>
      ...
      <div style={{backgroundColor:"#E8E9EB",padding:"24px",position:"relative",zIndex:1}}>
        <p style={{...inter,fontSize:"20px",color:"#636473",lineHeight:"30px",margin:0}}>Your <strong style={{color:"#040617"}}>{selected==="custom"?(custom||"$50"):selected}</strong> donation can help equip up to <strong style={{color:"#040617"}}>20</strong> students with school supplies, access to technology, and a safe learning environment — giving them the chance to thrive and build a better future.</p>
      </div>
    </div>
  );
}
...
function PersonalInfoStep({ form, setForm, errors, onBack, onNext, mobile=false }) {
  const upd = f => e => setForm({...form,[f]:e.target.value});
  const selectedCountry = COUNTRIES.find(c=>c.name===form.country);
  const hasStates = selectedCountry?.states?.length > 0;
  ...
  return (
    <div style={{backgroundColor:"#FFFDF9",border:"1px solid #E5E6EB",borderRadius:"20px",overflow:"hidden"}}>
      ...
      <div style={{padding:"24px",display:"flex",flexDirection:"column",gap:"20px"}}>
        ...
        {/* Country dropdown */}
        <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
          <label style={{...inter,fontSize:"16px",color:"#414651",letterSpacing:"0.16px",lineHeight:"24px"}}>Country</label>
          <div style={{backgroundColor:"#FFFDF9",border:`1px solid ${errors.country?"#EF4444":"#E5E6EB"}`,borderRadius:"8px",padding:"10px 14px",boxShadow:"0px 1px 2px rgba(10,13,18,0.05)"}}>
            <select value={form.country} onChange={e=>setForm({...form,country:e.target.value,state:""})}
              style={{...inter,width:"100%",border:"none",outline:"none",backgroundColor:"transparent",fontSize:"16px",color:form.country?"#040617":"#9CA3AF",cursor:"pointer"}}>
              <option value="">Select country...</option>
              {COUNTRIES.map(c=><option key={c.code} value={c.name}>{c.name}</option>)}
            </select>
          </div>
          {errors.country&&<span style={{...inter,fontSize:"14px",color:"#EF4444"}}>{errors.country}</span>}
        </div>

        <div style={{display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:"20px"}}>
          <InputField label="City" value={form.city} onChange={upd("city")} error={errors.city}/>

          {/* State — dropdown if country has states, else text input */}
          <div style={{display:"flex",flexDirection:"column",gap:"6px"}}>
            <label style={{...inter,fontSize:"16px",color:"#414651",letterSpacing:"0.16px",lineHeight:"24px"}}>{hasStates?"State / Parish":"State / Province"}</label>
            <div style={{backgroundColor:"#FFFDF9",border:`1px solid ${errors.state?"#EF4444":"#E5E6EB"}`,borderRadius:"8px",padding:"10px 14px",boxShadow:"0px 1px 2px rgba(10,13,18,0.05)"}}>
              {hasStates ? (
                <select value={form.state} onChange={upd("state")}
                  style={{...inter,width:"100%",border:"none",outline:"none",backgroundColor:"transparent",fontSize:"16px",color:form.state?"#040617":"#9CA3AF",cursor:"pointer"}}>
                  <option value="">Select...</option>
                  {selectedCountry.states.map(s=><option key={s} value={s}>{s}</option>)}
                </select>
              ) : (
                <input type="text" value={form.state} onChange={upd("state")} placeholder="Start Typing..."
                  style={{...inter,width:"100%",border:"none",outline:"none",backgroundColor:"transparent",fontSize:"16px",color:"#040617"}}/>
              )}
            </div>
            {errors.state&&<span style={{...inter,fontSize:"14px",color:"#EF4444"}}>{errors.state}</span>}
          </div>
        </div>
...
// ── Step 3 — Donate Method ────────────────────────────────────────────────────
function DonateMethodStep({ cardNumber,setCardNumber,cardExpiry,setCardExpiry,cardCvv,setCardCvv,error,loading,isProcessing,paymentSuccess,paymentResult,donationLabel,projectTitle,onBack,onSubmit,mobile=false }) {
  const expiryRef = useRef(null);
  const cvvRef    = useRef(null);
  const [highlighted, setHighlighted] = useState(null); // 'expiry' | 'cvv'
  ...
  const fieldStyle = (hl) => ({
    backgroundColor: highlighted===hl ? "#FFFBE6" : "#FFFDF9",
    border: `1px solid ${highlighted===hl ? "#FFD900" : "#E5E6EB"}`,
  ...
  if (paymentSuccess && paymentResult) return (
    <motion.div initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}}
      ...
      <div style={{backgroundColor:"#F5F3EE",borderRadius:"16px",padding:"20px",textAlign:"left",width:"100%",maxWidth:"360px"}}>
        {paymentResult.authorizationCode&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:"10px"}}><span style={{...inter,fontSize:"15px",color:"#6F7181"}}>Authorization Code</span><span style={{...inter,fontSize:"15px",fontWeight:600,color:"#040617"}}>{paymentResult.authorizationCode}</span></div>}
        {paymentResult.orderId&&<div style={{display:"flex",justifyContent:"space-between",marginBottom:"10px"}}><span style={{...inter,fontSize:"15px",color:"#6F7181"}}>Order ID</span><span style={{...inter,fontSize:"15px",fontWeight:600,color:"#040617"}}>{paymentResult.orderId}</span></div>}
        {paymentResult.cardBrand&&<div style={{display:"flex",justifyContent:"space-between"}}><span style={{...inter,fontSize:"15px",color:"#6F7181"}}>Card</span><span style={{...inter,fontSize:"15px",fontWeight:600,color:"#040617"}}>{paymentResult.cardBrand}</span></div>}
      </div>
      <p style={{...inter,fontSize:"14px",color:"#6F7181",margin:0}}>A confirmation has been sent to your email address.</p>
      <a href="/" style={{...inter,display:"inline-flex",alignItems:"center",gap:"8px",padding:"14px 32px",borderRadius:"14px",background:"#FFD900",color:"#040617",fontSize:"16px",fontWeight:600,textDecoration:"none"}}>Return to Home</a>
    </motion.div>
  );
    ...
    <div style={{backgroundColor:"#FFFDF9",border:"1px solid #E5E6EB",borderRadius:"20px",overflow:"hidden"}}>
      <div style={{display:"flex",borderBottom:"1px solid #E5E6EB"}}>
        {["card","paypal","inperson"].map(m=>(
          <button key={m} style={{...inter,flex:1,padding:"16px 10px",background:"none",border:"none",borderBottom:m==="card"?"2px solid #FFD900":"2px solid transparent",fontSize:mobile?"20px":"24px",color:"#040617",cursor:"pointer",opacity:m==="card"?1:0.75,textTransform:"capitalize"}}>
      ...
      <div style={{padding:"24px",display:"flex",flexDirection:"column",gap:"24px"}}>
        ...
        <div>
          ...
          <div style={{backgroundColor:"#FFFDF9",border:"1px solid #E5E6EB",borderRadius:"8px",padding:"10px 14px",boxShadow:"0px 1px 2px rgba(10,13,18,0.05)",display:"flex",alignItems:"center",gap:"10px"}}>
            <input type="text" placeholder="1234 5678 9012 3456" value={cardNumber} onChange={handleCardNumber} maxLength={19}
              style={{...inter,flex:1,border:"none",outline:"none",backgroundColor:"transparent",fontSize:"16px",color:"#040617",letterSpacing:"0.1em"}}/>
        ...
        <div style={{display:"flex",flexDirection:mobile?"column":"row",justifyContent:"space-between",alignItems:mobile?"stretch":"flex-end",gap:"20px",marginTop:"10px"}}>
          ...
          <button onClick={onSubmit} disabled={loading}
            ...
            {loading?(<><span>Processing</span><span style={{display:"flex",gap:"4px"}}>{[0,0.2,0.4].map((d,i)=><span key={i} style={{width:"6px",height:"6px",borderRadius:"50%",backgroundColor:"#040617",animation:`pulse 1.4s infinite ${d}s`,display:"inline-block"}}/>)}</span></>):"Donate Now"}
          </button>
        </div>
      </div>
    </div>
    ...
    <div style={{backgroundColor:"#FFFDF9",border:"1px solid #E5E6EB",borderRadius:"20px",overflow:"hidden"}}>
      ...
      <div style={{padding:"24px"}}>
        ...
        <div style={{border:"2px solid #E5E6EB",borderRadius:"16px",overflow:"hidden"}}>
          <iframe
            ref={el=>{if(el){const d=el.contentDocument||el.contentWindow?.document;if(d){d.open();d.write(redirectData);d.close();}}}}
...
export default function DonationForm() {
  ...
  useEffect(()=>{
    const handler = async (event) => {
      ...
      if (event.data.status === "3ds_complete" && event.data.spiToken) {
        setStep(5); // show processing inline
        setIsProcessing(true);
        await completePaymentInline(event.data.spiToken);
      } else if (event.data.status === "declined" || event.data.status === "error") {
        setPayError(event.data.message || "Authentication failed");
        setStep(3);
  ...
  const completePaymentInline = async (spiToken) => {
    try {
      const meta = donationMeta || {};
      const cr = await fetch("/api/complete",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({spiToken, donationMeta:meta}),
      });
      ...
      if (cd.success && cd.approved) {
        ...
        setStep(3); // show success in step 3 slot
      } else {
  ...
  const handlePayment = async ()=>{
    ...
    try {
      ...
      const res = await fetch("/api/donate",{
        method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({
          amount:numericAmount(), currency:"840",
          cardPan:cardNumber.replace(/\s/g,""), cardCvv,
          cardExpiration:`${yy}${mm}`,
          cardholderName:`${form.firstName} ${form.lastName}`.trim(),
          firstName:form.firstName, lastName:form.lastName,
          email:form.email, phone:"",
          address:form.address1, city:form.city,
          state:form.state, postalCode:form.zip,
          countryCode:"840", donationType:tab, message:"",
  ...
  const renderRightPanel = (mobile=false) => (
    <AnimatePresence mode="wait">
      <motion.div key={step} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} transition={{duration:0.3}} style={mobile?{width:"100%"}:{}}>
        {step===1&&<AmountStep tab={tab} setTab={setTab} selected={selected} setSelected={setSelected} custom={custom} setCustom={setCustom} error={errors.amount} onNext={handleStep1Next} mobile={mobile}/>}
        {step===2&&<PersonalInfoStep form={form} setForm={setForm} errors={errors} onBack={()=>{setStep(1);setErrors({})}} onNext={()=>{if(validateStep2())setStep(3)}} mobile={mobile}/>}
        {(step===3||step===5)&&<DonateMethodStep
          cardNumber={cardNumber} setCardNumber={setCardNumber}
          cardExpiry={cardExpiry} setCardExpiry={setCardExpiry}
          cardCvv={cardCvv} setCardCvv={setCardCvv}
          error={payError} loading={loading}
          isProcessing={isProcessing}
          paymentSuccess={paymentSuccess} paymentResult={paymentResult}
          donationLabel={donationLabel} projectTitle={selectedProject?.title||"this project"}
          onBack={()=>{setStep(2);setPayError("")}} onSubmit={handlePayment} mobile={mobile}/>}
        {step===4&&redirectData&&<AuthStep redirectData={redirectData}/>}
      </motion.div>
    </AnimatePresence>
  );

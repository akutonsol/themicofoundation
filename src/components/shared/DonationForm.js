...
function AuthStep({ redirectData, onCancel, processing }) {
  ...
  const showIframe = !isTimedOut && !processing;

  // Write the RedirectData into an about:blank iframe via contentDocument.
  // about:blank INHERITS this page's origin (unlike srcDoc's opaque origin),
  // so the 3DS form can POST to the cross-origin /Api/spi/Conductor endpoint and
  // the response renders in the iframe instead of being downloaded as a file.
  useEffect(() => {
    if (!showIframe || !redirectData) return;
    if (wroteFor.current === redirectData) return; // write once per token
    ...
    try {
      ...
      console.error("[3DS] iframe write failed:", e?.message || e);
    }
  }, [showIframe, redirectData]);
    ...
    <div style={{position:"fixed",inset:0,zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center",backgroundColor:"rgba(4,6,23,0.75)",backdropFilter:"blur(4px)"}}>
      <div style={{width:"min(520px, 95vw)",backgroundColor:"white",borderRadius:"20px",overflow:"hidden",boxShadow:"0 24px 80px rgba(0,0,0,0.35)"}}>
        ...
        {isTimedOut ? (
          ...
          /* Keep the 3DS iframe visible and interactive the whole time — never
             hide it on a timer or the challenge form becomes unusable. We write
             the RedirectData into this about:blank iframe via contentDocument
             (see effect above) so the cross-origin Conductor POST renders here. */
          <iframe
            ref={iframeRef}
            src="about:blank"
            frameBorder="0"

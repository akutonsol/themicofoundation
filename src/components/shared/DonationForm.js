...
function ProjectCard({ projects, currentProject, onPrev, onNext, mobile=false }) {
  ...
  if (mobile) return (
    <div style={{display:"flex",flexDirection:"column",gap:"20px",width:"100%",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{position:"relative",zIndex:1}}>
        <p style={{...inter,fontSize:"32px",color:"#5EDA71",textAlign:"center",letterSpacing:"-0.32px",lineHeight:"46px",margin:"0 0 4px"}}>{pct}%</p>
    ...
    <div style={{backgroundColor:"#FFFDF9",border:"1px solid #E5E6EB",borderRadius:"20px",padding:"16px",display:"flex",flexDirection:"column",gap:"32px",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",alignItems:"flex-start",justifyContent:"space-between",position:"relative",zIndex:1}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:"8px"}}>
          <img src={imgLocation} alt="" style={{width:"24px",height:"24px"}}/><span style={{...inter,fontSize:"16px",color:"#6F7181"}}>{p.location}</span>
        </div>
      </div>
  ...
  if (mobile) return (
    <div style={{width:"100%",backgroundColor:"#FFFDF9",borderRadius:"16px",overflow:"hidden",border:"1px solid #E5E6EB",display:"flex",flexDirection:"column"}}>
      <div style={{display:"flex",borderBottom:"1px solid #E5E6EB"}}>
        <button style={{flex:1,padding:"10px",...inter,fontSize:"24px",lineHeight:"38px",letterSpacing:"0.24px",color:"#040617",background:"none",border:"none",borderBottom:tab==="once"?"2px solid #FFD900":"2px solid transparent",cursor:"pointer",opacity:tab==="once"?1:0.8}} onClick={()=>{setTab("once");setSelected("$50")}}>Once</button>
        <button style={{flex:1,padding:"10px",...inter,fontSize:"24px",lineHeight:"38px",letterSpacing:"0.24px",color:"#040617",background:"none",border:"none",borderBottom:tab==="monthly"?"2px solid #FFD900":"2px solid transparent",cursor:"pointer",opacity:tab==="monthly"?1:0.8}} onClick={()=>{setTab("monthly");setSelected("$50/Month")}}>Monthly</button>
      </div>
      <div style={{padding:"24px",display:"flex",flexDirection:"column",gap:"16px"}}>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px"}}>
          {amounts.map(a=><button key={a} onClick={()=>setSelected(a)} style={{cursor:"pointer",border:`1px solid ${selected===a?"#FFD900":"#E5E6EB"}`,background:selected===a?"#FFD900":"#FFFDF9",borderRadius:"12px",padding:"10px 32px",...inter,fontSize:"20px",lineHeight:"30px",color:"#040617",textAlign:"center",transition:"all 0.2s"}}>{a}</button>)}
        </div>
        <button onClick={()=>setSelected("custom")} style={{cursor:"pointer",border:`1px solid ${selected==="custom"?"#FFD900":"#E5E6EB"}`,background:selected==="custom"?"#FFD900":"#FFFDF9",borderRadius:"12px",padding:"10px 32px",...inter,fontSize:"20px",color:"#040617",textAlign:"center",transition:"all 0.2s"}}>Custom</button>
  ...
  return (
    <div style={{backgroundColor:"#FFFDF9",border:"1px solid #E5E6EB",borderRadius:"20px",overflow:"hidden",display:"flex",flexDirection:"column"}}>
      ...
      <div style={{padding:"24px",display:"flex",flexDirection:"column",gap:"20px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:mobile?"1fr":"1fr 1fr",gap:"20px"}}>
          <InputField label="Country" value={form.country} onChange={upd("country")} error={errors.country}/>
...
// ── Step 3 — Donate Method (card) ─────────────────────────────────────────────
function DonateMethodStep({ cardNumber,setCardNumber,cardExpiry,setCardExpiry,cardCvv,setCardCvv,error,loading,donationLabel,projectTitle,onBack,onSubmit,mobile=false }) {
  return (
    <div style={{backgroundColor:"#FFFDF9",border:"1px solid #E5E6EB",borderRadius:"20px",overflow:"hidden",display:"flex",flexDirection:"column"}}>
      <div style={{display:"flex",borderBottom:"1px solid #E5E6EB"}}>
      ...
      <div style={{padding:"24px",display:"flex",flexDirection:"column",gap:"24px"}}>
        ...
        <div style={{display:"flex",flexDirection:mobile?"column":"row",justifyContent:"space-between",alignItems:mobile?"stretch":"flex-end",gap:"20px",marginTop:"10px"}}>
          ...
          <button onClick={onSubmit} disabled={loading}
            ...
            {loading?(
              <><span>Processing</span><span style={{display:"flex",gap:"4px"}}>
                {[0,0.2,0.4].map((d,i)=><span key={i} style={{width:"6px",height:"6px",borderRadius:"50%",backgroundColor:"#040617",animation:`pulse 1.4s infinite ${d}s`,display:"inline-block"}}/>)}
              </span></>
            ):"Donate Now"}
          </button>
        </div>
  ...
  const handlePayment = async ()=>{
    ...
    try {
      ...
      if (data.spiToken){
        ...
        if(cd.success&&cd.approved){window.location.href="/donate-result?status=success";}
  ...
  return (
    <section style={{backgroundColor:"#FFFDF9",position:"relative",overflow:"hidden"}}>
        ...
        <div className="don-grid">
          ...
          <AnimatePresence mode="wait">
            <motion.div key={step} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} transition={{duration:0.3}}>
              {step===1&&<AmountStep tab={tab} setTab={setTab} selected={selected} setSelected={setSelected} custom={custom} setCustom={setCustom} error={errors.amount} onNext={handleStep1Next}/>}
              {step===2&&<PersonalInfoStep form={form} setForm={setForm} errors={errors} onBack={()=>{setStep(1);setErrors({})}} onNext={()=>{if(validateStep2())setStep(3)}}/>}
              {step===3&&<DonateMethodStep cardNumber={cardNumber} setCardNumber={setCardNumber} cardExpiry={cardExpiry} setCardExpiry={setCardExpiry} cardCvv={cardCvv} setCardCvv={setCardCvv} error={payError} loading={loading} donationLabel={donationLabel} projectTitle={selectedProject?.title||"this project"} onBack={()=>{setStep(2);setPayError("")}} onSubmit={handlePayment}/>}
      ...
      <div className="don-mobile" style={{flexDirection:"column",gap:"24px",padding:"48px 24px",position:"relative",zIndex:1}}>
        ...
        <StepIndicator currentStep={displayStep} mobile/>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={`m-${step}`} initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-20}} transition={{duration:0.3}} style={{width:"100%"}}>
            {step===1&&<AmountStep tab={tab} setTab={setTab} selected={selected} setSelected={setSelected} custom={custom} setCustom={setCustom} error={errors.amount} onNext={handleStep1Next} mobile/>}
            {step===2&&<PersonalInfoStep form={form} setForm={setForm} errors={errors} onBack={()=>{setStep(1);setErrors({})}} onNext={()=>{if(validateStep2())setStep(3)}} mobile/>}
            {step===3&&<DonateMethodStep cardNumber={cardNumber} setCardNumber={setCardNumber} cardExpiry={cardExpiry} setCardExpiry={setCardExpiry} cardCvv={cardCvv} setCardCvv={setCardCvv} error={payError} loading={loading} donationLabel={donationLabel} projectTitle={selectedProject?.title||"this project"} onBack={()=>{setStep(2);setPayError("")}} onSubmit={handlePayment} mobile/>}
            {step===4&&redirectData&&<AuthStep redirectData={redirectData}/>}
          </motion.div>
        </AnimatePresence>

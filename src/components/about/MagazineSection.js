...
const staticMagazines = [
  {
    _id: "static-1",
    title: "The Mico Foundation Magazine",
    issue: "Annual General Meeting 2025",
...
function MagazineReader({ magazine, onClose }) {
  ...
  const next = useCallback(() => {
    if (currentPage < totalPages - 1) goTo(currentPage + 1, 1);
    ...
    >
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 24px', borderBottom: '1px solid rgba(255,255,255,0.06)', flexShrink: 0 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
          ...
          {/* Zoom controls */}
          <button onClick={() => setZoom(z => Math.max(0.5, z - 0.25))} style={{ background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: '8px', padding: '8px', cursor: 'pointer', color: 'white', display: 'flex' }}>
            <ZoomOut size={16} />
          </button>
          <span style={{ ...inter, fontSize: '12px', color: 'rgba(255,255,255,0.4)', minWidth: '36px', textAlign: 'center' }}>{Math.round(zoom * 100)}%</span>
          <button onClick={() => setZoom(z => Math.min(2.5, z + 0.25))} style={{ background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: '8px', padding: '8px', cursor: 'pointer', color: 'white', display: 'flex' }}>
            <ZoomIn size={16} />
          </button>
          <button onClick={() => setZoom(1)} style={{ background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: '8px', padding: '8px', cursor: 'pointer', color: 'white', display: 'flex' }}>
            <Maximize2 size={16} />
          </button>

          {/* Close */}
          <button onClick={onClose} style={{ background: 'rgba(255,255,255,0.08)', border: 'none', borderRadius: '8px', padding: '8px 14px', cursor: 'pointer', color: 'white', display: 'flex', alignItems: 'center', gap: '6px', fontFamily: inter.fontFamily, fontSize: '13px' }}>
            <X size={16} />
            Close
          </button>
        </div>
      ...
      <div style={{ display: 'flex', flex: 1, overflow: 'hidden' }}>
        ...
        {/* Page viewer */}
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', overflow: 'hidden', position: 'relative', padding: '24px' }}>
          ...
          {/* Page image */}
          <div style={{ overflow: 'hidden', maxHeight: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              ...
              >
                {currentImg && (
                  <img
                    ...
                    style={{ maxHeight: 'calc(100vh - 160px)', maxWidth: '100%', objectFit: 'contain', borderRadius: '8px', boxShadow: '0 32px 80px rgba(0,0,0,0.6)', display: 'block' }}
                  />
                )}
                {currentImg?.caption && (
      ...
      try {
        ...
        if (data?.length > 0) {
          setMagazines(data.map(mag => ({
            ...
            title: mag.title,
            issue: mag.issue || '',
            coverUrl: mag.coverImage ? urlFor(mag.coverImage).width(800).url() : (mag.pages?.[0]?.image ? urlFor(mag.pages[0].image).width(800).url() : ''),

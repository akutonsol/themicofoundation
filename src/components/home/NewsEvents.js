      ...
      try {
        ...
        if (featuredData) {
          setFeatured({
            ...
            // FIX: slug is already a plain string from our query
            slug: featuredData.slug,
          })
          console.log('✅ Loaded featured article from CMS')
        }
        ...
        if (sideData && sideData.length > 0) {
          ...
          console.log('✅ Loaded', sideData.length, 'side articles from CMS')
        }
      } catch (error) {
      ...
      <div className="news-desktop">
        ...
        <div className="news-grid" style={{ position:'relative', zIndex:1 }}>
          ...
          <motion.div initial={{ opacity:0, y:20 }} whileInView={{ opacity:1, y:0 }} viewport={{ once:true }} transition={{ duration:0.6 }}
            ...
            {/* FIX: type-based routing */}
            <a href={getItemHref(featured)} style={{ ...inter, display:'inline-flex', alignItems:'center', gap:'8px', backgroundColor:'#FFD900', color:'#040617', fontSize:'14px', fontWeight:600, height:'46px', padding:'0 24px', borderRadius:'14px', textDecoration:'none', width:'240px', justifyContent:'center' }}>
              Read Full
              <img src={imgArrow} alt="" style={{ width:'24px', height:'24px', objectFit:'contain' }} />
            </a>
          </motion.div>
      ...
      <div className="news-mobile" style={{ flexDirection:'column', gap:'18px', alignItems:'center', padding:'48px 24px 60px', position:'relative', zIndex:1 }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:'30px', width:'100%' }}>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'24px', width:'100%' }}>
            ...
            {/* FIX: type-based routing */}
            <a href={getItemHref(featured)} style={{ ...inter, display:'inline-flex', alignItems:'center', justifyContent:'center', gap:'8px', backgroundColor:'#FFD900', color:'#040617', fontSize:'14px', fontWeight:600, height:'46px', padding:'0 24px', borderRadius:'14px', textDecoration:'none', width:'240px', alignSelf:'flex-start' }}>
              Read Full
              <img src={imgArrow} alt="" style={{ width:'24px', height:'24px', objectFit:'contain' }} />
            </a>
          </div>
          ...
          <div style={{ display:'flex', flexDirection:'column', gap:'24px', width:'100%' }}>
            {sideArticles.map((article) => (
              // FIX: type-based routing
              <a key={article.id} href={getItemHref(article)} style={{ display:'flex', flexDirection:'column', gap:'24px', paddingBottom:'16px', borderBottom:'1px solid rgba(225,225,225,0.3)', textDecoration:'none', cursor:'pointer' }}>

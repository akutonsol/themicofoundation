    ...
    return (
      <div style={{ display:'flex', flexDirection:'column', gap:'20px', width:'100%', position:'relative', overflow:'hidden' }}>
        ...
        <div style={{ position:'relative', zIndex:1 }}>
          ...
          <a href={`/projectdetail?slug=${project.slug}`} style={{ ...inter, fontSize:'24px', color:'#6F7181', letterSpacing:'0.24px', lineHeight:'38px', textDecoration:'underline' }}>Learn more</a>
        </div>
        <div style={{ position:'relative', zIndex:1 }}>
...
function PersonalInfoStep({ form, setForm, errors, mobile = false, onBack, onNext }) {
  ...
  return (
    <div style={{ backgroundColor:'#FFFDF9', border:'1px solid #E5E6EB', borderRadius:'20px', overflow:'hidden', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ padding:'24px', display:'flex', flexDirection:'column', gap:'20px' }}>{content}</div>
    </div>
  )
}
...
function PaymentField({ label, placeholder = 'Start Typing...', value, onChange, error, type = 'text' }) {
  ...
  const formatCVC = (value) => value.replace(/[^0-9]/gi, '').substring(0, 3)
  const handleChange = (e) => {
    ...
    else if (type === 'cvc') formatted = formatCVC(e.target.value)
...
function DonateMethodStep({ paymentMethod, setPaymentMethod, payment, setPayment, errors, donationLabel, projectTitle, mobile = false, onBack, onDonate, isProcessing, paymentSuccess, redirectData }) {
  ...
  return (
    <div style={{ backgroundColor:'#FFFDF9', border:'1px solid #E5E6EB', borderRadius:'20px', overflow:'hidden', display:'flex', flexDirection:'column' }}>
      ...
      <div style={{ padding:'24px', display:'flex', flexDirection:'column', gap:'24px' }}>
        ...
        {errors.submit && (
          <p style={{ ...inter, fontSize:'16px', color:'#EF4444', textAlign:'center', margin:0 }}>{errors.submit}</p>
        )}

        {!paymentSuccess && !redirectData && (
              ...
              {isProcessing ? (
                <>
                  ...
                  <span style={{ display:'flex', gap:'4px' }}>
                    <span style={{ width:'6px', height:'6px', borderRadius:'50%', backgroundColor:'#040617', animation:'pulse 1.4s infinite', display:'inline-block' }} />
                    <span style={{ width:'6px', height:'6px', borderRadius:'50%', backgroundColor:'#040617', animation:'pulse 1.4s infinite 0.2s', display:'inline-block' }} />
                    <span style={{ width:'6px', height:'6px', borderRadius:'50%', backgroundColor:'#040617', animation:'pulse 1.4s infinite 0.4s', display:'inline-block' }} />
                  </span>
                </>
              ) : 'Donate Now'}
            </button>
...
export default function DonationForm() {
  ...
  const validateStep3 = () => {
    ...
    if (!payment.expiry.trim()) { newErrors.expiry = 'Expiry date is required' }
      ...
      if (expiry < new Date()) { newErrors.expiry = 'Card has expired' }
    }
  ...
  const submitDonation = async () => {
    if (paymentMethod !== 'card') {
      setErrors({ submit: 'Please use a credit/debit card to donate online.' })
      return
    }
    try {
      ...
      const payload = {
        amount,
        currency: '840',
        cardPan: payment.cardNumber.replace(/\s/g, ''),
        cardCvv: payment.cvc,
      ...
      const response = await fetch('/api/donate', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) })
      const result = await response.json()
      if (!response.ok) throw new Error(result.error || 'Failed to process donation')
      const meta = result.donationMeta || {}
      setDonationMeta(meta)
      try { sessionStorage.setItem('donationMeta', JSON.stringify(meta)) } catch {}
      if (result.requiresRedirect && result.redirectData) {
        setRedirectData(result.redirectData)
